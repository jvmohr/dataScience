...
# To grab the current table and not past ones

year = '2020' # switch this to command line argument
league = 'mls'
...
url = "https://www.espn.com/soccer/standings/_/league/USA.1/season/{}".format(year) # change this
...
table_df.to_csv(os.path.join('data', league, 'tables', year + '_tables.csv'), index=False)

# Add it to past tables
all_tables_df = pd.read_csv(os.path.join('data', league, 'tables', '2003_2019_tables.csv'))
all_tables_df = all_tables_df.append(table_df, ignore_index=True)
all_tables_df.to_csv(os.path.join('data', league, 'tables', 'all_tables.csv'), index=False)
